...
```
In the `boot_straps_df`, the 2.5% quantile for r^2 is `r round(r_low_quantile, digit = 3)` and the 97.5% quantile for r^2 is `r round(r_high_quantile, digit = 3)`. The 2.5% quantile for log(β^0 ∗ β^1) is `r round(log_low_quantile, digit = 3)` and the 97.5% quantile for r^2 is `r round(log_high_quantile, digit = 3)`. Next, the distribution of 5000 bootstrap estimate is plotted in order to identify the  2.5% and 97.5% quantiles for a 95% confidence interval by the code chunk below.
...
```
The plots above show the density distributions of two quantities estimated from 5,000 bootstrap samples from the 2017 Central Park weather data. The *r_squared distribution* has a relatively heavy tail extending to low values while the *log(β^0∗β^1) distribution* has a relatively normal distribution. Both the distribution features may be related to the frequency in which large outliers are included in the bootstrap samples.